...
import { SlideIssue } from "@/components/features/Result/Flow/Detail/SlideIssue";
import { SlideData, IssueData } from "@/types/Result";

export const SlideInfo = ({ slide, i }: { slide: SlideData; i: number }) => {
  return (
    <Box p={24}>
      <Flex ta={"center"} direction={"column-reverse"} justify={"center"} align={"center"}>
      ...
      {slide.issues.length > 0 ? (
        slide.issues.map((issue: IssueData, issueIndex: number) => <SlideIssue issue={issue} key={issueIndex} />)
      ) : (
