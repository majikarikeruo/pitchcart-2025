...
import { Paper, Title, Stack, Group, Text, Badge, Loader, Center, Switch } from "@mantine/core";
    ...
    try {
      ...
      const filteredHistory = history.filter((h) => {
        ...
        if (h.createdAt instanceof Date) {
          ...
          date = new Date((h.createdAt as any).toDate());
        }
      ...
      <Stack gap="lg">
        <Group justify="space-between">
          <div>
            ...
            <Group gap="xs" mt="xs">
              ...
              {improvement !== 0 && (
                <Badge color={improvement > 0 ? "teal" : "red"} variant="light" size="sm">
                  {improvement > 0 ? "+" : ""}
          ...
          <Switch label="全カテゴリ表示" checked={showAllCategories} onChange={(e) => setShowAllCategories(e.currentTarget.checked)} size="sm" />
        </Group>
        ...
        <div style={{ width: "100%", height: 300 }}>
          <ResponsiveContainer>
            <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Line type="monotone" dataKey="totalScore" stroke="#228be6" strokeWidth={3} dot={{ fill: "#228be6", r: 4 }} />

              {showAllCategories && (
                <>
                  <Line type="monotone" dataKey="contentScore" stroke="#20c997" strokeWidth={2} dot={{ fill: "#20c997", r: 3 }} strokeDasharray="5 5" />
                  <Line type="monotone" dataKey="designScore" stroke="#fd7e14" strokeWidth={2} dot={{ fill: "#fd7e14", r: 3 }} strokeDasharray="5 5" />
                  <Line type="monotone" dataKey="persuasivenessScore" stroke="#e03131" strokeWidth={2} dot={{ fill: "#e03131", r: 3 }} strokeDasharray="5 5" />
                  <Line type="monotone" dataKey="technicalScore" stroke="#9775fa" strokeWidth={2} dot={{ fill: "#9775fa", r: 3 }} strokeDasharray="5 5" />
                </>
              )}
            </LineChart>
          </ResponsiveContainer>
        ...
        <Group gap="xl" justify="center">
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <Text fw={700}>{(data.reduce((sum, d) => sum + d.totalScore, 0) / data.length).toFixed(1)}点</Text>
          </div>
        </Group>
      </Stack>
    </Paper>
