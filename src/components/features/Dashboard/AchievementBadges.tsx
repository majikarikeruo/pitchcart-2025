...
import { Paper, Title, Stack, Group, Badge, Text, SimpleGrid, Card, ThemeIcon, Progress, Tooltip, Center, Loader } from "@mantine/core";
import { IconTrophy, IconStar, IconFlame, IconTarget, IconTrendingUp, IconBrain, IconUsers, IconQuestionMark, IconCheck } from "@tabler/icons-react";
      ...
      const achievementList: Achievement[] = [
        ...
        // 質問関連
        {
          ...
          unlocked: (() => {
            ...
            const unanticipated = allFeedbacks.reduce((sum, f) => sum + f.questionsReceived.filter((q) => !q.wasAnticipated).length, 0);
        ...
        // 満足度関連
        {
          ...
          progress: allFeedbacks.length > 0 ? allFeedbacks.reduce((sum, f) => sum + f.outcomes.overallSuccess, 0) / allFeedbacks.length : 0,
          maxProgress: 4.5,
        },
      ];
          ...
          {achievements.map((achievement) => (
            <Tooltip key={achievement.id} label={achievement.description} position="top" withArrow>
              ...
              >
                <Stack gap="sm">
                  <Group justify="space-between">
                    <ThemeIcon color={achievement.unlocked ? achievement.color : "gray"} variant={achievement.unlocked ? "filled" : "light"} size="lg">
                      {achievement.icon}
                    </ThemeIcon>
                  ...
                  {!achievement.unlocked && (
                    <div>
                      ...
                      <Progress value={(achievement.progress / achievement.maxProgress) * 100} color={achievement.color} size="xs" />
                    </div>
                  )}
                </Stack>
              </Card>
