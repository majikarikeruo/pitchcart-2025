...
export class AnalysisService {
  // 新規分析結果の保存
  async saveAnalysis(userId: string, presentationId: string, presentationTitle: string, analysisData: AnalysisResponse): Promise<string> {
    try {
      // 既存のバージョンを取得して次のバージョン番号を決定
      const previousVersions = await this.getAnalysisHistory(userId, presentationId);
      const nextVersion = previousVersions.length > 0 ? Math.max(...previousVersions.map((v) => v.version)) + 1 : 1;
  ...
  // 分析履歴の取得（プレゼンテーション別）
  async getAnalysisHistory(userId: string, presentationId?: string): Promise<AnalysisHistory[]> {
    try {
      let q;
      if (presentationId) {
        // インデックスエラーを回避するため、一旦orderByを削除
        q = query(collection(db, "analysisHistory"), where("userId", "==", userId), where("presentationId", "==", presentationId));
      } else {
        // インデックスエラーを回避するため、一旦orderByを削除
        q = query(collection(db, "analysisHistory"), where("userId", "==", userId), limit(50));
      }
      ...
      return results.sort((a, b) => {
        if (presentationId) {
          ...
          const dateA = a.createdAt?.toDate?.() || new Date(a.createdAt.toDate());
          const dateB = b.createdAt?.toDate?.() || new Date(b.createdAt.toDate());
  ...
  async getFeedback(analysisId: string): Promise<PracticeFeedback[]> {
    try {
      // インデックスエラーを回避
      const q = query(collection(db, "practiceFeedback"), where("analysisId", "==", analysisId));
  ...
  // バージョン比較データの生成
  private generateComparison(currentData: AnalysisResponse, previousVersion: AnalysisHistory) {
