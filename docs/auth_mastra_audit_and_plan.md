...
- `.env` は `.gitignore` に記載して Git 管理から除外します（ローカルには残します）。本番・開発環境では環境変数（Secrets）として設定します。
...
- `.env` はリポジトリ（Git）には無く、ローカル開発環境およびデプロイ環境のシークレットで管理されている。
...
1. 機密の Git 管理除外（Git から `.env` を削除 `git rm --cached .env` し、`.gitignore` を確認。ローカルファイルは保持する）。
