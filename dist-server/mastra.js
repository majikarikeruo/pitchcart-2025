...
import { llmEvaluatePersonaWithOpts, llmMergeConsensusWithOpts } from './llm.js';
 ...
 */
export async function mastraEvaluatePersonas(input, personas, opts) {
    // Currently, we fall back to direct, parallel LLM calls.
    // A future Mastra implementation would replace this with a `Workflow.run()`.
    const tasks = personas.map((cfg) => llmEvaluatePersonaWithOpts(input, cfg, opts || {}).catch(() => fallbackPersona(cfg.persona_id)));
 ...
 */
export async function mastraMergeConsensus(personasOut, opts) {
