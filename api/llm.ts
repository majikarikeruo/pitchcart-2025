...
import type { PersonaOutput, Consensus, StructureSimulation, EmotionalArc, SlideStruct } from './schema.js';
import {
  PersonaOutputSchema,
  ConsensusSchema,
  EmotionalArcSchema,
  StructureSimulationSchema, // Added import
  validateAndRepairJson,
  fallbackPersona,
} from './schema.js';

// --- DUMMY DATA ---
const DUMMY_EMOTIONAL_ARC: EmotionalArc = {
  title: 'プレゼンテーションの感情アーク分析',
  summary:
    'このプレゼンテーションは、序盤で聞き手の課題への共感を呼び、中盤で解決策への期待感を高め、終盤で具体的な行動を促す納得感を与える構成になっています。特に、顧客の成功事例を提示するスライドで感情のピークを迎えます。',
  points: [
...
export async function llmEvaluatePersona(input: any, persona: PersonaConfig): Promise<PersonaOutput> {
  return llmEvaluatePersonaWithOpts(input, persona, {});
}

export async function llmEvaluatePersonaWithOpts(input: any, persona: PersonaConfig, opts: LlmPersonaOpts): Promise<PersonaOutput> {
...
export async function llmMergeConsensus(personas: PersonaOutput[]): Promise<Consensus> {
...
export async function llmSimulateStructure(
  slidesStruct: { index: number; title: string; texts: string[] }[],
  opts: LlmMergeOpts
          ...
          resolve({
            ...
            steps: [
              ...
              },
              {
                ...
                slide_indices: [3, 4, 5],
                description:
                  '次に、スライド3,4,5で、なぜその課題が重要なのか、市場にどんなペインが存在するのかをデータや顧客の声で裏付けます。',
              },
              {
                title: 'Example (具体例)',
                slide_indices: [6, 7, 8, 9],
                description:
                  'そして、スライド6-9で、プロダクトのデモや具体的なユースケースを示し、どのように課題を解決するのかを具体的に伝えます。',
              },
              {
                title: 'Point (結論の再提示)',
                slide_indices: [10, 11, 12],
                description:
                  '最後に、スライド10-12で、ビジネスモデルやチームを紹介し、この事業が成功する必然性を改めて強調して締めくくります。',
              },
            ],
          }),
        1000
...
export async function llmAnalyzeEmotionalArc(
  slides_struct: SlideStruct[],
  opts: LlmMergeOpts,
  ...
  const user = `
# 分析対象スライド
${slides_struct.map((s: SlideStruct) => `## スライド ${s.index}: ${s.title}\n${s.texts.join('\n')}`).join('\n\n')}
