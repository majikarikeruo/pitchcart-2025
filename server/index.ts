...
process.on('uncaughtException', (err) => {
  try { console.error('[server] uncaughtException:', err); } catch {}
});
...
function heuristicScoreFromSlides(
  summary: string,
  slidesText: string,
  slidesStruct: any[] | undefined,
  weighting: PersonaConfig['weighting']
) {
  const text = `${summary || ''}\n${slidesText || ''}`.trim();
...
async function analyze(input: any, runtime: RuntimeOpts) {
  ...
  if (runtime.useMastra) {
    ...
    personaOutputs = await mastraEvaluatePersonas(input, personasCfg, {
      ...
      const tasks = personasCfg.map((cfg) => personaEvaluate(input, cfg, ac.signal, runtime).then(
        (p) => ({ status: 'fulfilled' as const, value: p }),
        (e) => ({ status: 'rejected' as const, reason: e, persona_id: cfg.persona_id })
      ));
      const settled = await Promise.all(tasks);
      return settled.map((r, idx) => (r.status === 'fulfilled' ? r.value : fallbackPersona((r as any).persona_id || personasCfg[idx]?.persona_id || `persona_${idx}`)));
    });
  } else {
    const ac = new AbortController();
    const tasks = personasCfg.map((cfg) => personaEvaluate(input, cfg, ac.signal, runtime).then(
    ...
    const settled = await Promise.all(tasks);
    personaOutputs = settled.map((r, idx) => (r.status === 'fulfilled' ? r.value : fallbackPersona((r as any).persona_id || personasCfg[idx]?.persona_id || `persona_${idx}`)));
  }

  // Validate/repair each persona output; fallback only if invalid
  const repaired = personaOutputs.map((p) =>
    validateAndRepairJson(p, PersonaOutputSchema, 'persona') ?? fallbackPersona(p.persona_id)
  );
...
const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (ctype.startsWith('multipart/form-data')) {
        ...
        if (speechFile?.filepath) {
          try { speechText = await fsPromises.readFile(speechFile.filepath, 'utf-8'); } catch(e) { console.error("Could not read speech file", e)}
        }

        const getFirst = (v: any) => (Array.isArray(v) ? v[0] : v);
        const meta = {
          target_person: String(getFirst((fields as any).target_person) || ''),
          goal: String(getFirst((fields as any).goal) || ''),
          industry: String(getFirst((fields as any).industry) || ''),
        };
        ...
        const runtime = getRuntimeOptsFromBody({
          use_llm: getFirst((fields as any).use_llm),
          use_mastra: getFirst((fields as any).use_mastra),
          llm_provider: getFirst((fields as any).llm_provider),
          persona_model: getFirst((fields as any).persona_model),
          merge_model: getFirst((fields as any).merge_model),
          persona_timeout_ms: getFirst((fields as any).persona_timeout_ms),
          merge_timeout_ms: getFirst((fields as any).merge_timeout_ms),
        });
  ...
  if (req.method === 'POST' && pathname === '/api/analyze/stream') {
    try {
      ...
      if (ctype.startsWith('multipart/form-data')) {
        ...
        const pptx = Array.isArray((files as any).file) ? (files as any).file[0] : (files as any).file;
        const speech = Array.isArray((files as any).speech_text) ? (files as any).speech_text[0] : (files as any).speech_text;
        let slidesText = '';
        let slidesStruct: any[] = [];
        if (pptx?.filepath && String(pptx.originalFilename || '').toLowerCase().endsWith('.pptx')) {
          ...
          if (String(process.env.LOG_ANALYSIS || '').toLowerCase() === 'true') {
            const titles = ext.slides.map((s: any) => String(s?.title || '')).filter(Boolean).slice(0, 5);
            console.log(`[server] extract pptx: slides=${ext.slides.length} titles=${JSON.stringify(titles)}`);
          }
          lastExtractDebug = { slidesCount: Array.isArray(ext.slides) ? ext.slides.length : 0, sampleTitles: Array.isArray(ext.slides) ? ext.slides.map((s:any)=>String(s?.title||'')).filter(Boolean).slice(0,3) : [] };
        }
        let speechText = '';
        if (speech?.filepath) {
          try { speechText = await fsPromises.readFile(speech.filepath, 'utf-8'); } catch {}
        }
        const getFirst2 = (v: any) => (Array.isArray(v) ? v[0] : v);
        const meta = {
          target_person: String(getFirst2((fields as any).target_person) || ''),
          goal: String(getFirst2((fields as any).goal) || ''),
          industry: String(getFirst2((fields as any).industry) || ''),
        };
        const summarySeed = `${meta.industry} ${meta.goal} ${meta.target_person}`.trim();
      ...
      const tasks = personasCfg.map((cfg) =>
        personaEvaluate(body, cfg, ac.signal, runtime).then(
          (p) => ({ type: 'persona' as const, data: p, persona_id: cfg.persona_id }),
          () => ({ type: 'persona' as const, data: fallbackPersona(cfg.persona_id), persona_id: cfg.persona_id })
        )
      );

      // As-completed streaming
      const pending = new Set(tasks);
      const safeWrite = (fn: () => void) => { try { if (!res.writableEnded) fn(); } catch {} };
      tasks.forEach((t) => {
        t.then((msg) => {
      ...
      const personaOutputs: PersonaOutput[] = settled.map((r, idx) =>
        (r.status === 'fulfilled' ? r.value.data : null) || fallbackPersona(personasCfg[idx].persona_id)
      );
      
      let consensus = createConsensus(personaOutputs);
      if (runtime.useMastra) {
        try { consensus = await mastraMergeConsensus(personaOutputs, { provider: runtime.provider, model: runtime.mergeModel, timeoutMs: runtime.mergeTimeoutMs }); } catch {}
      } else if (runtime.useLLM) {
        try { consensus = await llmMergeConsensusWithOpts(personaOutputs, { provider: runtime.provider, model: runtime.mergeModel, timeoutMs: runtime.mergeTimeoutMs }); } catch {}
      }

      safeWrite(() => writeSse(res, 'message', { type: 'consensus', data: consensus }));
      safeWrite(() => writeSse(res, 'done', {}));
      if (!res.writableEnded) res.end();
      res.on('error', () => { try { res.end(); } catch {} });
  ...
  if (req.method === 'POST' && pathname === '/api/analyze/emotional_arc') {
    try {
      ...
      const slides_struct = jsonBody.slides_struct as SlideStruct[];
      if (!slides_struct) {
